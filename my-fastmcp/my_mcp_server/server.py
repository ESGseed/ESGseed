...
class SentenceSuggestion(BaseModel):
    ...
    ifrs_codes: List[str]             # 예: ["S2-5", "S2-15"]
    overall_status: Literal["pass", "partial", "fail"]
...
2. 정량적 데이터가 누락된 부분은 LLM이 가정하지 말고, [필수 입력: 20XX년까지 Scope 1, 2 배출량 XX% 감축]과 같은 명확한 사용자 입력 요청 문구로 대체하여 문단을 완성하세요.
3. 'IFRS S2'나 문단 번호는 문단 안에 직접 언급하지 마세요.
4. 결과는 하나의 문단만 출력하고, 불릿 포인트나 추가 설명은 작성하지 마세요.
5. **[필수 제약] 오직 요청된 IFRS S2 코드({req.code})와 관련된 내용만 포함해야 하며, 거버넌스, 전략, 위험 관리, 지표 및 목표 등 다른 핵심 IFRS S2 영역의 내용은 일절 포함하지 마세요. (예: S2-5(거버넌스) 문단에는 S2-29(지표) 내용을 넣지 마세요)**
"""
    return prompt.strip()
